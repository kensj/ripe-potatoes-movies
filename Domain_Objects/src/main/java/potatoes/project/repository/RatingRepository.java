...
public interface RatingRepository extends JpaRepository<Rating, Integer>{
	...
	@Query("select r.score from Rating r where r.score>=1 and r.score<2 and r.content=?1")
	List<Rating> findOneStar(Content c);
	@Query("select r.score from Rating r where r.score>=2 and r.score<3 and r.content=?1")
	List<Rating> findTwoStar(Content c);
	@Query("select r.score from Rating r where r.score>=3 and r.score<4 and r.content=?1")
	List<Rating> findThreeStar(Content c);
	@Query("select r.score from Rating r where r.score>=4 and r.score<5 and r.content=?1")
